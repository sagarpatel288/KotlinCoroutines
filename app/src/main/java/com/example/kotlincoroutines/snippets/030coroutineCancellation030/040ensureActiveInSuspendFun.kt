...
class EnsureActiveInSuspendFun {
    ...
    fun nestedScopesAndCoroutines() = runBlocking {
        val outerJob = CoroutineScope(Dispatchers.Default).launch {
            ...
            delay(2000L)
            innerJob.cancel()
            // If the `suspendFunction` which is a part of the `innerJob` does not actively check for the cancellation,
            // we may get some print statement from it even after cancelling the `innerJob`!
     ...
     *
     * In this example, we actively check for the cancellation.
     * Hence, as soon as the parent coroutine (from which this `suspendFunction` has been called) is cancelled,
     * any ongoing coroutine execution will also be cancelled (stopped).
     *
     */
...
fun main() {
    EnsureActiveInSuspendFun().nestedScopesAndCoroutines()
}
